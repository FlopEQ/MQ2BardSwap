...
VOID AddElement(PCHAR szLine)
{
	...
	pExcluded=pElement;
	strncpy_s(pElement->Song, szLine ? szLine : "", _TRUNCATE);
...
VOID LoadINI()
{
   ...
   for (int i=0;InstrumentList[i]!=NULL;i++) {
      ...
      if(!strcmp(szTemp,"DISABLED")){
         ...
         InstrumentSettings[i].Command[0]=0;
         InstrumentSettings[i].Enabled=false;
      } else {
         strncpy_s(InstrumentSettings[i].Command, szTemp, _TRUNCATE);
...
PLUGIN_API void OnPulse()
{
	...
	if (!bSwapEnabled || GetGameState() !=GAMESTATE_INGAME || !pCharInfo || !pCharInfo->pSpawn || pCharInfo->standstate != STANDSTATE_STAND)
		return;
	...
	if (pCurrentSong!=pOldSong) {
		...
		if (bMeleeSwap && iCurrentSwap!=5 && InstrumentSettings[5].Enabled && InstrumentSettings[5].Command[0]) {
	...
	if (DelayCounter<=::GetTickCount64() && SwapTo!=iCurrentSwap && InstrumentSettings[SwapTo].Enabled && InstrumentSettings[SwapTo].Command[0]) {
